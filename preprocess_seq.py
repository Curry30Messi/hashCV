...
def seq_standardize(folder):
    ...
    for filename in files:
        ...
        if filename[-6:] == ".fastq":
            result.to_csv('data_std/' + filename[:-6] + '_stdCSV.csv', index=True, encoding='gbk')
        elif filename[-3:] == ".fa":
            result.to_csv('data_std/' + filename[:-3] + '_stdCSV.csv', index=True, encoding='gbk')
        elif filename[-4:] == ".fna":
            result.to_csv('data_std/' + filename[:-4] + '_stdCSV.csv', index=True, encoding='gbk')
